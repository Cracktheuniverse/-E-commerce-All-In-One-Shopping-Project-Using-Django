...
# def offer(request,data=None):
#     if data==None:
#         sopa=Product.objects.filter(category='H')
#     elif data=='gojaraj' or data ='Royal':
...
def wkidswear(request,data=None):
 if data == None:
     wkidswears = Product.objects.filter(category='WKW')
 elif data == 'levis' or data == 'Lee' or data == 'Allen':
     wkidswears = Product.objects.filter(category='WKW').filter(brand=data)
 elif data == 'below':
     wkidswears = Product.objects.filter(category='WKW').filter(discounted_price__lt=600)
 elif data == 'above':
     wkidswears = Product.objects.filter(category='WKW').filter(discounted_price__gt=600)

 return render(request, 'app/wkidswear.html',{'wkidswears':wkidswears})
...
def Foods(request,data=None):
 if data == None:
     Foods = Product.objects.filter(category='F')
 elif data == 'Oil' or data == 'Soaps' or data == 'PlasticProduct':
     Foods = Product.objects.filter(category='F').filter(brand=data)
 elif data == 'below':
     Foods = Product.objects.filter(category='F').filter(discounted_price__lt=1200)
 elif data == 'above':
     Foods = Product.objects.filter(category='F').filter(discounted_price__gt=1200)
...
def checkout(request):
    ...
    for p in cart_product:
        tempamount = (p.quantity * p.product.discounted_price)
        amount1 += tempamount
    totalamount = amount1 + shipping_amount
        
        
        
    amount = totalamount * 100 #100 here means 1 dollar,1 rupree if currency INR
    if amount < 100:
        amount = 100
    ...
    print(response,"****************************************")
        ...
        # Perform any additional actions for order processing

        # Redirect the user or render a success page

    
    
        
    
    return render(request, 'app/checkout.html',{'add':add,'totalamount':totalamount,'cart_items':cart_items,'response':response })
